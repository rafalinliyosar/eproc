...
export default class DisplaySupp extends React.Component {
    constructor(props) {
        super(props);
        // console.log(props)
        this.state = {
    ...
    async componentDidMount() {
        ...
        let response = await fetch("http://localhost:8000/api/displaysupp", {
			method: "POST",
			headers: myHeaders,
               ...
               <table  >
                ...
                <tbody>
                        ...
                        {this.state.records.map((record,key) => {
                            return (
                                <tr key={key} >
                                    <td>{record.supp_id}</td>
                                    <td>{record.supp_name}</td>
                                    <td>{record.supp_type}</td>
                                     <td>
