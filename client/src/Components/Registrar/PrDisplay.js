...
export default class PrDisplay extends React.Component {
    constructor(props) {
        super(props);
        // console.log(props)
        this.state = {
    ...
    fetchdata = async () => {
        ...
        let response = await fetch("http://localhost:8000/api/prdisplayreg", {
            method: "POST",
            headers: myHeaders,
    ...
    approve = async (id) => {
        ...
        console.log("id", id);
        await fetch("http://localhost:8000/api/approvereg", {
            method: "POST",
            headers: myHeaders,
            body: body
        }).catch((error) => console.log("error", error));
