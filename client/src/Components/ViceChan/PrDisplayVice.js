...
export default class PrDisplayVice extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            records: [],
    ...
    fetchdata = async () => {
        ...
        let response = await fetch("http://localhost:8000/api/prdisplayvic", {
            method: "POST",
            headers: myHeaders,
    ...
    approve = async (id) => {
        ...
        console.log("id", id);
        await fetch("http://localhost:8000/api/approvevic", {
            method: "POST",
            headers: myHeaders,
            body: body
        }).catch((error) => console.log("error", error));
